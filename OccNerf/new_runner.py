...
class Runer:
    ...
    def __init__(self, options):
        ...
        self.log_path = osp.join(self.opt.log_dir, self.opt.model_name + 'exp-{}'.format(
            time.strftime("%Y_%m_%d-%H_%M", time.localtime())))

        print('log path:', self.log_path)
        os.makedirs(osp.join(self.log_path, 'eval'), exist_ok=True)
        os.makedirs(osp.join(self.log_path, 'vis_feature'), exist_ok=True)
        ...
        self.models["encoder"] = networks.Encoder_res101(self.opt.input_channel, path=None,
                                                         network_type=self.opt.encoder)
        ...
        if self.opt.position == 'embedding1':
            self.parameters_to_train += [
                {'params': self.models["depth"]._3DCNN.parameters(), 'lr': self.opt.de_lr,
                 'weight_decay': self.opt.weight_decay},
                {'params': self.models["depth"].embeddingnet.parameters(), 'lr': self.opt.en_lr,
                 'weight_decay': self.opt.weight_decay}]
        ...
        if self.opt.dataset == 'nusc':
            nusc = NuScenes(version='v1.0-mini', dataroot=osp.join(self.opt.dataroot, 'nuscenes'), verbose=False)
        else:
            nusc = None
    ...
    def val(self, save_image=True):
        ...
        with torch.no_grad():
            ...
            for idx, data in enumerate(loader):
                ...
                #
                #     # pdb.set_trace()
                #     cv2.imwrite('{}/visual_rgb_depth/{}-{}.jpg'.format(self.log_path, self.local_rank, idx),
                #                 surround_view)
                #
                #     vis_dic = {}
                ...
                data_point = {'index': data['index'], 'feature': features[0]}
                vis_feature.append(data_point)

                if idx % 100 == 0 and not idx== 0:
                    with open(osp.join(self.log_path, 'vis_feature', '{}_{}.pkl'.format(self.local_rank, idx)),
                              'wb') as f:
                        pickle.dump(vis_feature, f)
                    print("{} ends".format(idx))
                    vis_feature = []

        with open(osp.join(self.log_path, 'vis_feature', '{}_final.pkl'.format(self.local_rank)),
                  'wb') as f:
            pickle.dump(vis_feature, f)
    ...
    def compute_self_supervised_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.use_semantic and scale == 0:
                ...
                target_semantic = F.interpolate(target_semantic.unsqueeze(1).float(), size=pred_semantic.shape[1:3],
                                                mode="nearest").squeeze(1)

                semantic_loss += self.sem_criterion(pred_semantic.view(-1, self.opt.semantic_classes),
                                                    target_semantic.view(-1).long())
    ...
    def log_time(self, batch_idx, len_loader, duration, loss_dict):
        ...
        if self.local_rank == 0:
            ...
            self.log_print_train(
                print_string.format(self.epoch + 1, self.opt.num_epochs, batch_idx + 1, len_loader, samples_per_sec,
                                    loss_info,
