...
class VolumeDecoder(nn.Module):
    ...
    def grid_sampler(self, xyz, *grids, align_corners=True, avail_mask=None, vis=False):
        ...
        if xyz_sem is None:
            ...
            # sem: torch.Size([31920347, 18])
            # geo: torch.Size([109463141, 1])
            # ind_geo
            # torch.Size([1, 1, 1, 109463141, 3])
            # ind_sem
            # torch.Size([1, 1, 1, 31920347, 3])
            # torch.Size([1, 1, 300, 300, 24])
            # torch.Size([1, 18, 300, 300, 24])
    ...
    def forward(self, features, inputs, outputs={}, is_train=True, Voxel_feat_list=None, no_depth=False):
        ...
        if is_train:
            ...
            # depth: torch.Size([345600])
            # depth: torch.Size([6, 1, 180, 320])
